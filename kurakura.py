...
def detailExceptionByType(exctype):
    if exctype == "NameError":
        return "Unknown variable name! Make sure you reference only variables that exist."
...
def exceptionHook(exctype, value, traceback: types.TracebackType): 
    ...
    print(chalk.blackBright("Error type: ") + chalk.bold.magenta(exceptionTypeName) + " - \"" + chalk.redBright(value) + "\"")
    print(chalk.bold.magenta("At: ") + traceback.tb_frame.f_code.co_filename + ":" + str(traceback.tb_lineno))
    print("  - " +  chalk.blue(readSpecifiedLine(traceback.tb_frame.f_code.co_filename, traceback.tb_lineno).strip()))
    print(chalk.bold.blue("Explanation: ") + detailExceptionByType(exceptionTypeName or "Exception"))
    print("\n")
    print(chalk.bold.green("Reporting the error?"))
    print(chalk.bold.green("Please include the following error ID:"))
    errorID = str(hex(int.from_bytes(exceptionTypeName.encode(), 'big'))) + "-" + str(hex(traceback.tb_lineno)) + "-" + str(hex(int.from_bytes(anonymizeFile(traceback.tb_frame.f_code.co_filename).encode(), 'big'))) + "-" + str(hex(int.from_bytes(str(value).encode(), 'big')))
...
if os.path.isfile('beautify.exe'):
    ...
    halo = Halo(text='Downloading beautifier...', spinner='dots8', animation='marquee', force=True)
    halo.start()
    s = req.get('https://github.com/uncrustify/uncrustify/releases/download/uncrustify-0.78.1/uncrustify-0.78.1_f-win64.zip', allow_redirects=True)
    open('uncrustify-0.78.1_f-win64.zip', 'wb').write(s.content)
    halo.succeed(text='Downloaded beautifier.')
    halo = Halo(text='Extracting beautifier...', spinner='dots8', animation='marquee', force=True)
    halo.start()
...
def parse(fileName, path):
    ...
    open(('out/src/' + path + '/' + '.'.join(fileName.split('.')[:-1]) + '.cpp').replace('//', '/'), 'w').write(content)
    print('  - ' + chalk.bold.yellow('Writing header file...'))
    open(('out/include/' + path + '/' + '.'.join(fileName.split('.')[:-1]) + '.h').replace('//', '/'), 'w').write(header)

    newTime = time.time()
    print('  - ' + chalk.bold.green('Done!') + ' Took ' + chalk.bold.magenta(str(round(newTime - oldTime, 2))) + ' seconds.')
    ...
    for arg in args:
        if type(arg) == ast.Constant:
            ...
            if type(arg) == ast.Name:
                ...
                args1.append(arg.func.id + "(" + transformInternalRebuildArgs(arg.args) + ")")
...
def TransformFunctionDefinition(node: ast.FunctionDef):
    ...
    if inClass or node.name.startswith("__"):
        if node.name == "__init__":
                    ...
                    if type(x.value) == ast.Call:
                        if type(x.value.func) == ast.Attribute:
                            if x.value.func.attr.startswith("this."):
                                x.value.func.attr = x.value.func.attr.replace("this.", "this->")
            ...
            header += typeReturned + " " + node.name + "(" + transformInternalRebuildArgs(node.args.args) + ");\n"
            content += typeReturned + " " + node.name + "(" + transformInternalRebuildArgs(node.args.args) + ") {\n"
        ...
        if type(node.returns) == ast.Constant:
            print("   - " + chalk.bold.blueBright("Function ") + node.name + chalk.bold.blueBright(" returns ") + ("None" if not node.returns else internalCatchify(lambda: str(node.returns.id), lambda: transformInternalStringifyConstant(node.returns))) + "...")
            if type(node.returns.value) == str:
                header += "char *" + node.name + "(" + transformInternalRebuildArgs(node.args.args) + ");\n"
                content += "char *" + node.name + "(" + transformInternalRebuildArgs(node.args.args) + ") {\n"
...
def TransformFunctionCall(node: ast.Call):
    ...
    if node.func.id == "print":
        content += "std::cout << " + transformInternalRebuildArgs(node.args) + " << std::endl;\n"
    elif node.func.id.startswith("namespaces"):
        content += node.func.id.replace("namespaces.", "").replace(".", "::") + "(" + transformInternalRebuildArgs(node.args) + ");\n"
    else:
    ...
    if type(node.value) == ast.Call:
        ...
        if type(node.value.func) == ast.Name:
            ...
            if _prior_chain[0] == "new":
                ...
                content += chained + " " + node.targets[0].id + "(" + transformInternalRebuildArgs(node.value.args) + ");\n"
                ...
                content += "auto " + node.targets[0].id + " = " + chained + "(" + transformInternalRebuildArgs(node.value.args) + ");\n"
                if not inFunction and not isDefined and not valu in defined_external_vars:
            ...
            if _prior_chain[0] == "new":
                ...
                content += ("auto " if not isDefined else "") + node.targets[0].id + " = " + chained + "(" + transformInternalRebuildArgs(node.value.args) + ");\n"
                if not inFunction and not isDefined and not valu in defined_external_vars:
                    header += "extern " + chained + " " + node.targets[0].id + ";\n" 
                    defined_external_vars.append(node.targets[0].id)
    else:
            if type(node.targets[0]) == ast.Attribute:
                if not isDefined:
                    var_types[node.targets[0].attr] = transformInternalGetTypeName(node.value)
                    defined_variables.append(node.targets[0].attr)
                    defined_global_vars.append(node.targets[0].attr)
                if type(node.value.value) == str:
                    content += ("char *" if not isDefined else "") + node.targets[0].attr + " = \"" + node.value.value + "\";\n"
                elif type(node.value.value) == bool:
                    content += ("bool " if not isDefined else "") + node.targets[0].attr + " = " + ("true" if node.value.value else "false") + ";\n"
                else:
                    content += (transformInternalGetTypeName(node.value) + " " if not isDefined else "") + node.targets[0].attr + " = " + str(node.value.value) + ";\n"
                if not inFunction and not isDefined and not valu in defined_external_vars:
                    header += "extern " + var_types[node.targets[0].attr] + " " + node.targets[0].attr + ";\n"
                ...
                if type(node.value) == ast.UnaryOp:
                    ...
                    content += transformInternalGetTypeName(node.value) + " " + node.targets[0].id + " = " + transformInternalGetOpName(node.value.op) + ";\n"
                elif type(node.value) == ast.Constant:
                    if type(node.value.value) == str:
                        node.value.value = node.value.value.replace("\\", "\\\\").replace("\"", "\\\"").replace("\n", "\\n").replace("\t", "\\t").replace("\r", "\\r").replace("\'", "\\'")
                        content += ("char *" if not isDefined else "") + node.targets[0].id.strip() + " = \"" + node.value.value + "\";\n"
                        if not inFunction and not isDefined and not valu in defined_external_vars:
                            header += "extern char *" + node.targets[0].id + ";\n"
                            defined_external_vars.append(node.targets[0].id)
                    elif type(node.value.value) == bool:
                        content += ("bool " if not isDefined else "") + node.targets[0].id + " = " + ("true" if node.value.value else "false") + ";\n"
                        if not inFunction and not isDefined and not valu in defined_external_vars:
                            header += "extern bool " + node.targets[0].id + ";\n"
                            defined_external_vars.append(node.targets[0].id)
                    else:
                        content += (transformInternalGetTypeName(node.value) + " " if not isDefined else "") + node.targets[0].id + " = " + str(node.value.value) + ";\n"
                        if not inFunction and not isDefined and not valu in defined_external_vars:
                            header += "extern " + transformInternalGetTypeName(node.value) + " " + node.targets[0].id + ";\n"
                            defined_external_vars.append(node.targets[0].id)
            else:
                if type(node.value) == ast.Name:
                    if not isDefined:
                        ...
                        defined_variables.append(node.targets[0].id)
                        defined_global_vars.append(node.targets[0].id)
                    content += (var_types[node.value.id] + " " if not isDefined else "") + node.targets[0].id + " = " + node.value.id + ";" + ("\n" if inFunction else "")
                    if not inFunction and not isDefined and not valu in defined_external_vars:
                        header += "extern " + var_types[node.value.id] + " " + node.targets[0].id + ";\n"
                        defined_external_vars.append(node.targets[0].id)
                    ...
                    if chain[0] == "namespaces":
                        ...
                        content += (("auto " if not inferredType else inferredType + " ") if not isDefined else "") + node.targets[0].id + " = " + ".".join(chain[1:]).replace(".", "::") + "(" + transformInternalRebuildArgs(node.value.args) + ");\n"
                        if not inFunction and not isDefined and not valu in defined_external_vars:
                            header += "extern " + (("auto " if not inferredType else inferredType + " ") if not isDefined else "") + node.targets[0].id + ";\n"
                            defined_external_vars.append(node.targets[0].id)
                    else:
                        if type(node.value.func) == ast.Attribute:
                            if not isDefined:
                                ...
                                defined_variables.append(node.targets[0].id)
                                defined_global_vars.append(node.targets[0].id)
                            content += (node.value.func.attr + " " if not isDefined else "") + node.targets[0].id + " = " + '.'.join(chain) + "(" + transformInternalRebuildArgs(node.value.args) + ");\n"
                            if not inFunction and not isDefined and not valu in defined_external_vars:
                                header += "extern " + node.value.func.attr + " " + node.targets[0].id + ";\n"
                        else:
                            if not isDefined:
                                var_types[node.targets[0].id] = node.value.func.id
                                defined_variables.append(node.targets[0].id)
                                defined_global_vars.append(node.targets[0].id)
                            content += (node.value.func.id + " " if not isDefined else "") + node.targets[0].id + " = " + '.'.join(chain) + "(" + transformInternalRebuildArgs(node.value.args) + ");\n"
                            if not inFunction and not isDefined and not valu in defined_external_vars:
                                header += "extern " + node.value.func.id + " " + node.targets[0].id + ";\n"
                                defined_external_vars.append(node.targets[0].id)
                elif type(node.value) == ast.BinOp:
                    if type(node.value.left) == ast.Name:
                        if not isDefined:
                            var_types[node.targets[0].id] = transformInternalGetTypeName(node.value.left)
                            defined_variables.append(node.targets[0].id)
                            defined_global_vars.append(node.targets[0].id)
                        if not hasattr(var_types, node.value.left.id):
                            var_types[node.value.left.id] = transformInternalGetTypeName(node.value.left)
                        ...
                        if type(node.value.right) == ast.Constant:
                            content += (var_types[node.value.left.id] + " " if not isDefined else "") + node.targets[0].id + " = " + node.value.left.id + " " + transformInternalGetOpName(node.value.op) + " " + str(node.value.right.value) + ";\n"
                            if not inFunction and not isDefined and not valu in defined_external_vars:
                                header += "extern " + var_types[node.value.left.id] + " " + node.value.left.id + ";\n"
                            ...
                            if type(node.value.right.operand) == ast.Constant:
                                content += (var_types[node.value.left.id] + " " if not isDefined else "") + node.targets[0].id + " = " + node.value.left.id + " " + transformInternalGetOpName(node.value.op) + " " + transformInternalGetOpName(node.value.right.op) + " " + str(node.value.right.operand.value) + ";\n"
                                if not inFunction and not isDefined and not valu in defined_external_vars:
                                    header += "extern " + var_types[node.value.left.id] + " " + node.value.left.id + ";\n"
                                    defined_external_vars.append(node.value.left.id)
                            elif type(node.value.right.operand) == ast.Name:
                                content += (var_types[node.value.left.id] + " " if not isDefined else "") + node.targets[0].id + " = " + node.value.left.id + " " + transformInternalGetOpName(node.value.op) + " " + transformInternalGetOpName(node.value.right.op) + " " + node.value.right.operand.id + ";\n"
                                if not inFunction and not isDefined and not valu in defined_external_vars:
                                    header += "extern " + var_types[node.value.left.id] + " " + node.value.left.id + ";\n"
                                    defined_external_vars.append(node.value.left.id)
                            else:
                                content += (var_types[node.value.left.id] + " " if not isDefined else "") + node.targets[0].id + " = " + node.value.left.id + " " + transformInternalGetOpName(node.value.op) + " " + transformInternalGetOpName(node.value.right.op) + ";\n"
                                if not inFunction and not isDefined and not valu in defined_external_vars:
                                    header += "extern " + var_types[node.value.left.id] + " " + node.value.left.id + ";\n"
                                    defined_external_vars.append(node.value.left.id)
                        else:
                            content += (var_types[node.value.left.id] + " " if not isDefined else "") + node.targets[0].id + " = " + node.value.left.id + " " + transformInternalGetOpName(node.value.op) + " " + node.value.right.id + ";\n"
                            if not inFunction and not isDefined and not valu in defined_external_vars:
                                header += "extern " + var_types[node.value.left.id] + " " + node.value.left.id + ";\n"
                                defined_external_vars.append(node.value.left.id)
                        ...
                        if not isDefined:
                            ...
                            defined_global_vars.append(node.targets[0].id)
                        if type(node.value.right) == ast.Constant:
                            content += (transformInternalGetTypeName(node.value.left) + " " if not isDefined else "") + node.targets[0].id + " = " + str(node.value.left.value) + " " + transformInternalGetOpName(node.value.op) + " " + str(node.value.right.value) + ";\n"
                            if not inFunction and not isDefined and not valu in defined_external_vars:
                                header += "extern " + transformInternalGetTypeName(node.value.left) + " " + node.value.left.id + ";\n"
                                defined_external_vars.append(node.value.left.id)
                        else:
                            content += (transformInternalGetTypeName(node.value.left) + " " if not isDefined else "") + node.targets[0].id + " = " + str(node.value.left.value) + " " + transformInternalGetOpName(node.value.op) + " " + node.value.right.id + ";\n"
                            if not inFunction and not isDefined and not valu in defined_external_vars:
                                header += "extern " + transformInternalGetTypeName(node.value.left) + " " + node.value.left.id + ";\n"
                                defined_external_vars.append(node.value.left.id)
                    else:
                        if not isDefined:
                            ...
                            defined_variables.append(node.targets[0].id)
                            defined_global_vars.append(node.targets[0].id)
                        content += (transformInternalGetTypeName(node.value) + " " if not isDefined else "") + node.targets[0].id + " = " + node.value.left.id + " " + transformInternalGetOpName(node.value.op) + " " + node.value.right.id + ";\n"
                else:
                    if not isDefined:
                        var_types[node.targets[0].id] = transformInternalGetTypeName(node.value)
                        defined_variables.append(node.targets[0].id)
                        defined_global_vars.append(node.targets[0].id)
                    if type(node.value) == ast.UnaryOp:
                        content += (transformInternalGetTypeName(node.value) + " " if not isDefined else "") + node.targets[0].id + " = " + transformInternalGetOpName(node.value.op) + node.value.operand.id + ";\n"
                        if not inFunction and not isDefined and not valu in defined_external_vars:
                            header += "extern " + transformInternalGetTypeName(node.value) + " " + node.value.operand.id + ";\n"
                            defined_external_vars.append(node.value.operand.id)
                    elif type(node.value) == ast.Constant:
                        if type(node.value.value) == str:
                            content += ("char *" if not isDefined else "") + node.targets[0].id + " = \"" + node.value.value + "\";\n"
                        elif type(node.value.value) == bool:
                            content += ("bool " if not isDefined else "") + node.targets[0].id + " = " + ("true" if node.value.value else "false") + ";\n"
                        else:
                            content += (transformInternalGetTypeName(node.value) + " " if not isDefined else "") + node.targets[0].id + " = " + str(node.value.value) + ";\n"
                        ...
                        if type(node.value) == ast.Constant:
                            if type(node.value.value) == str:
                                ...
                                content += (transformInternalGetTypeName(node.value) + " " if not isDefined else "") + node.targets[0].id + " = " + node.value.id + ";\n"
                                if not inFunction and not isDefined and not valu in defined_external_vars:
                                    header += "extern " + transformInternalGetTypeName(node.value) + " " + node.value.value + ";\n"
                        else:
                            if type(node.value) == ast.Name:
                                content += (var_types[node.value.id] + " " if not isDefined else "") + node.targets[0].id + " = " + node.value.id + ";\n"
                                if not inFunction and not isDefined and not valu in defined_external_vars:
                                    header += "extern " + var_types[node.value.id] + " " + node.value.id + ";\n"
                                    defined_external_vars.append(node.value.id)
                            else:
                                content += (transformInternalGetTypeName(node.value) + " " if not isDefined else "") + node.targets[0].id + " = " + node.value.id + ";\n"
                                if not inFunction and not isDefined and not valu in defined_external_vars:
                                    header += "extern " + transformInternalGetTypeName(node.value) + " " + node.value.id + ";\n"
                                    defined_external_vars.append(node.value.id)
            if not inFunction and not isDefined and not valu in defined_external_vars:
                if var_types[node.targets[0].id] == "char *":
                    ...
                    header += "extern " + var_types[node.targets[0].id] + " " + node.targets[0].id + ";\n"
                defined_external_vars.append(node.targets[0].id)
            return node
...
def TransformIf(node: ast.If):
    ...
    if isinstance(node, ast.If):
        if type(node.test) == ast.Compare:
            content += "if (" + transformInternalRebuildConstantOrDynamic(node.test.left) + " " + transformInternalGetOpName(node.test.ops[0]) + " " + transformInternalRebuildConstantOrDynamic(node.test.comparators[0]) + ") {\n"
            ...
            content += "if (" + '.'.join(chain[1:]) + "(" + transformInternalRebuildArgs(node.test.args) + ")) {\n"
        else:
            ...
            for node2 in node.orelse:
                if isinstance(node2, ast.If):
                    if type(node2.test) == ast.Compare:
                        content += "else if (" + transformInternalRebuildConstantOrDynamic(node2.test.left) + " " + transformInternalGetOpName(node.test.ops[0]) + " " + transformInternalRebuildConstantOrDynamic(node.test.comparators[0]) + ") {\n"
                    else:
...
def TransformAugAssign(node: ast.AugAssign):
    ...
    if type(node.target) == ast.Attribute:
        ...
        isDefined = True
    content += (node.target.id + " " if not isDefined else "") + transformInternalStringifyAugAssign(node) + " " + transformInternalStringifyAugSide(node) + ";\n"
    return node
...
def transformInternalInferType(node: ast.AST):
    if type(node) == ast.Constant:
        ...
        if type(node.right) == ast.Constant:
            ...
            val_r = val_l_1 + " " + transformInternalGetOpName(node.right.op) + " " + val_r_1
        ...
        return transformInternalInferType(node.left) + " " + transformInternalGetOpName(node.op) + " " + val_r
    elif type(node) == ast.UnaryOp:
        return transformInternalGetOpName(node.op) + " " + transformInternalInferType(node.operand)
    else:
        raise Exception("Unknown node type: " + node.__class__.__name__)

def TransformExpr(node: ast.Expr):
    print("   - " + chalk.bold.blueBright("Expression ") + ((node.value.func.id if hasattr(node.value.func, "id") else node.value.func.attr) if hasattr(node.value, "func") else "") + "...")
    ...
    if isinstance(node.value, ast.Call):
        if type(node.value.func) == ast.Attribute:
            ...
            if built_path[0] == "namespaces":
                content += ".".join(built_path[1:]).replace(".", "::") + "(" + transformInternalRebuildCallArgs(node.value.args) + ");\n"
            else:
                content += ".".join(built_path) + "(" + (transformInternalRebuildCallArgs(node.value.args) or "") + ");\n"
            return node
            ...
            if s.startswith("header_flag"):
                ...
                content += "std::cout << " + transformInternalRebuildCallArgs(node.value.args) + " << std::endl;\n"
            else:
                content += node.value.func.id + "(" + (transformInternalRebuildCallArgs(node.value.args) or "") + ");\n"
            return node
    elif isinstance(node.value, ast.BinOp):
        content += transformInternalInferType(node.value) + " " + node.value.left.id + " " + transformInternalGetOpName(node.value.op) + " " + node.value.right.id + ";\n"
    elif isinstance(node.value, ast.UnaryOp):
        content += transformInternalInferType(node.value) + " " + node.value.op + " " + node.value.operand.id + ";\n"
    if type(node.value) == ast.Call:
        if type(node.value.func) == ast.Attribute:
            ...
            if name.startswith("header_flag"):
                ...
                content += node.value.func.attr + "(" + (transformInternalRebuildCallArgs(node.value.args) or "") + ");\n"
        else:
            name = node.value.func.id
            if name == "print":
                content += "std::cout << " + transformInternalRebuildCallArgs(node.value.args) + " << std::endl;\n"
            else:
                content += name + "(" + (transformInternalRebuildCallArgs(node.value.args) or "") + ");\n"
    else:
